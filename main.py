...
from model import CONFIG, RNN_LSTM, TextCNN, MLP
import argparse
...
def parser_data():
    ...
    parser.add_argument(
        "-e", "--epoch", dest="epoch", type=int, default=20, help="epoch"
    )
    parser.add_argument(
        "-m",
        "--max_length",
...
if __name__ == "__main__":
    ...
    for each in tqdm(range(1, epoch + 1)):
        ...
        print(
            f"for epoch {each}, train_loss: {tr_loss:.4f}, train_acc: {tr_acc:.4f}, val_loss: {val_loss:.4f}, val_acc: {val_acc:.4f}, test_loss: {test_loss:.4f}, test_acc: {test_acc:.4f}, test_f1: {test_f1:.4f}"
        )
